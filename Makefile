...
.PHONY: catalog
catalog:  ## regenerate the typed catalog from Calcite's manifest
	cd js; pnpm manifest
	python -m spaday.cem spaday_calcite/custom-elements.json -o spaday_calcite/components.py
	python -m ruff format spaday_calcite/components.py
...
lint-py:  ## run python linter with ruff
	python -m ruff check spaday_calcite js/examples/build_pyodide_example.py
	python -m ruff format --check spaday_calcite js/examples/build_pyodide_example.py
...
fix-py:  ## fix python formatting with ruff
	python -m ruff check --fix spaday_calcite
	python -m ruff format spaday_calcite
...
test-py:  ## run python tests
	python -m pytest -v spaday_calcite/tests

# alias
tests-py: test-py

coverage-py:  ## run python tests and collect test coverage
	python -m pytest -v spaday_calcite/tests --cov=spaday_calcite --cov-report term-missing --cov-report xml
...
pyodide-example: build  ## build the standalone Pyodide example into dist/lite
	test -n "$(firstword $(wildcard dist/spaday_calcite-*.whl))"
	rm -rf dist/lite dist/pyodide-deps
	mkdir -p dist/lite dist/pyodide-deps
	python -m pip download --no-deps --only-binary=:all: --platform pyemscripten_2026_0_wasm32 --python-version 314 --implementation cp --abi cp314 --dest dist/pyodide-deps "spaday==0.10.0"
	python js/examples/build_pyodide_example.py dist/lite "$(firstword $(wildcard dist/spaday_calcite-*.whl))" dist/pyodide-deps
...
test-pyodide-example: pyodide-example  ## run the standalone Pyodide example in Chromium
	...
	cd js; SPADAY_CALCITE_PYODIDE_ONLY=1 pnpm exec playwright test tests/pyodide.spec.js
