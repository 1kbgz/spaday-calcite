...
def main(output: Path, calcite_wheel: Path, dependency_dir: Path) -> None:
    spaday_wheel = one_wheel(dependency_dir, "spaday")
    wheels = {
        "calcite": copy_wheel(calcite_wheel, output),
        "spaday": copy_wheel(spaday_wheel, output),
    }
    extract_assets(calcite_wheel, "spaday_calcite", output / "components" / "calcite")
