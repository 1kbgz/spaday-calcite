...
const PACKAGES = ["@esri/calcite-components"];
const OUT = "../spaday_calcite/custom-elements.json";
// Internal calendar renderers require private JS context and are not public declarative components.
...
const UNPUBLISHED = fs.existsSync("tools/unpublished.json")
  ...
  : {};
// the platform and Calcite's framework base contribute nothing an author sets
const PLATFORM = new Set(["HTMLElement", "PublicLitElement"]);
...
function resolve(decl, owner, missing) {
  ...
  // a name is the default slot, which the manifest schema spells "" (Calcite spells it "default"); an
...
for (const name of PACKAGES) {
  ...
  for (const mod of owner.manifest.modules ?? []) {
    const declarations = (mod.declarations ?? [])
      ...
      .filter((decl) => !OMITTED.has(decl.tagName));
